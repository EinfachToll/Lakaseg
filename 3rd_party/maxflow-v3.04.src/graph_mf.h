...
#ifndef __GRAPHMF_H__
#define __GRAPHMF_H__
...
// captype_mf: type of edge capacities (excluding t-links)
// tcaptype: type of t-links (edges between nodes and terminals)
// flowtype_mf: type of total flow
//
// Current instantiations are in instances.inc
template <typename captype_mf, typename tcaptype, typename flowtype_mf> class Graph_mf
{
public:
	typedef enum
	{
	...
	Graph_mf(int node_num_max, int edge_num_max, void (*err_function)(const char *) = NULL);

	// Destructor
	~Graph_mf();

	// Adds node(s) to the graph. By default, one node is added (num=1); then first call returns 0, second call returns 1, and so on. 
	// If num>1, then several nodes are added, and node_id_mf of the first one is returned.
	// IMPORTANT: see note about the constructor 
	node_id_mf add_node(int num = 1);

	// Adds a bidirectional edge between 'i' and 'j' with the weights 'cap' and 'rev_cap'.
	// IMPORTANT: see note about the constructor 
	void add_edge(node_id_mf i, node_id_mf j, captype_mf cap, captype_mf rev_cap);
	...
	void add_tweights(node_id_mf i, tcaptype cap_source, tcaptype cap_sink);
	...
	flowtype_mf maxflow(bool reuse_trees = false, Block<node_id_mf>* changed_list = NULL);

	// After the maxflow is computed, this function returns to which
	// segment the node 'i' belongs (Graph_mf<captype_mf,tcaptype,flowtype_mf>::SOURCE or Graph_mf<captype_mf,tcaptype,flowtype_mf>::SINK).
	//
	...
	termtype what_segment(node_id_mf i, termtype default_segm = SOURCE);
...
public:
	...
	//
	// Advantage compared to deleting Graph_mf and allocating it again:
	...
	void get_arc_ends(arc_id a, node_id_mf& i, node_id_mf& j); // returns i,j to that a = i->j
	...
	tcaptype get_trcap(node_id_mf i); 
	// returns residual capacity of arc a
	captype_mf get_rcap(arc* a);
	...
	void set_trcap(node_id_mf i, tcaptype trcap); 
	void set_rcap(arc* a, captype_mf rcap);
	...
	// Example usage:
	//
	//		typedef Graph_mf<int,int,int> G;
	//		G* g = new Graph_mf(nodeNum, edgeNum);
	//		Block<G::node_id_mf>* changed_list = new Block<G::node_id_mf>(128);
	//
	//		... // add nodes and edges
	//
	...
	//			{
	//				G::node_id_mf i = *ptr; assert(i>=0 && i<nodeNum);
	...
	void remove_from_changed_list(node_id_mf i) 
	{ 
...
private:
	...
	struct arc
	{
		...
		captype_mf		r_cap;		// residual capacity
	};
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline typename Graph_mf<captype_mf,tcaptype,flowtype_mf>::node_id_mf Graph_mf<captype_mf,tcaptype,flowtype_mf>::add_node(int num)
{
	assert(num > 0);
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::add_tweights(node_id_mf i, tcaptype cap_source, tcaptype cap_sink)
{
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::add_edge(node_id_mf _i, node_id_mf _j, captype_mf cap, captype_mf rev_cap)
{
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline typename Graph_mf<captype_mf,tcaptype,flowtype_mf>::arc* Graph_mf<captype_mf,tcaptype,flowtype_mf>::get_first_arc()
{
	return arcs;
}

template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline typename Graph_mf<captype_mf,tcaptype,flowtype_mf>::arc* Graph_mf<captype_mf,tcaptype,flowtype_mf>::get_next_arc(arc* a) 
{
	return a + 1; 
}

template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::get_arc_ends(arc* a, node_id_mf& i, node_id_mf& j)
{
	assert(a >= arcs && a < arc_last);
	i = (node_id_mf) (a->sister->head - nodes);
	j = (node_id_mf) (a->head - nodes);
}

template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline tcaptype Graph_mf<captype_mf,tcaptype,flowtype_mf>::get_trcap(node_id_mf i)
{
	assert(i>=0 && i<node_num);
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline captype_mf Graph_mf<captype_mf,tcaptype,flowtype_mf>::get_rcap(arc* a)
{
	assert(a >= arcs && a < arc_last);
	return a->r_cap;
}
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::set_trcap(node_id_mf i, tcaptype trcap)
{
	assert(i>=0 && i<node_num); 
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::set_rcap(arc* a, captype_mf rcap)
{
	assert(a >= arcs && a < arc_last);
	a->r_cap = rcap;
}
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline typename Graph_mf<captype_mf,tcaptype,flowtype_mf>::termtype Graph_mf<captype_mf,tcaptype,flowtype_mf>::what_segment(node_id_mf i, termtype default_segm)
{
	if (nodes[i].parent)
	{
...
template <typename captype_mf, typename tcaptype, typename flowtype_mf> 
	inline void Graph_mf<captype_mf,tcaptype,flowtype_mf>::mark_node(node_id_mf _i)
{
	node* i = nodes + _i;
	if (!i->next)
	{
